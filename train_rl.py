...
env = DummyVecEnv([lambda: DoSDetectionEnv(dataset_path)])  # Vectorized environment for faster training
...
model = PPO(
    ...
    clip_range=0.2,           # Smaller clipping to prevent instability
    ent_coef=0.01,            # Increase entropy coefficient for better exploration
    vf_coef=0.3,              # Reduce value function impact to decrease loss
    max_grad_norm=0.5,        # Prevent exploding gradients
    verbose=1
)

# Train RL Model with More Steps
model.learn(total_timesteps=100000)  # Increase steps for better learning
